...
public class PasteHandler {
	...
	final static Logger logger = Logger.getLogger(PasteHandler.class);
	...
	public void execute(IWorkbench iWorkbench, Shell shell,
		...
		if (activePart.getElementId().equals(Constants.FILE_TABLE_VIEW_ID)) {
			...
			iEventBroker.send(Constants.FILE_TABLE_PASTE_EVENT_BUS_NAME, new UID().toString());

		} else if (activePart.getElementId().equals(Constants.GBOX_TABLE_VIEW_ID)) {

			iEventBroker.send(Constants.GBOX_PASTE_EVENT_BUS_NAME, new UID().toString());
