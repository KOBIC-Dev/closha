...
public class NewFolderHandler {
	...
	final static Logger logger = Logger.getLogger(NewFolderHandler.class);
	...
	public void execute(IWorkbench iWorkbench, Shell shell,
		...
		logger.info(activePart.getLabel() + "\t" + activePart.getElementId() + "\t" + object);

		if (activePart.getElementId().equals(Constants.FILE_TABLE_VIEW_ID)) {

			iEventBroker.send(Constants.FILE_TABLE_NEW_FOLDER_EVENT_BUS_NAME, new UID().toString());

		} else if (activePart.getElementId().equals(Constants.GBOX_TABLE_VIEW_ID)) {

			iEventBroker.send(Constants.GBOX_NEW_FOLDER_EVENT_BUS_NAME, new UID().toString());
