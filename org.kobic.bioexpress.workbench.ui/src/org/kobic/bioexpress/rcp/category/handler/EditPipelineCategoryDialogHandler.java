...
public class EditPipelineCategoryDialogHandler {
			...
			if (dialog.open() == Window.OK) {
				...
				if (dialog.getResult() == 1) {
					...
					if (categoryType.equals(Constants.PIPELINE_LABEL)) {
						...
						MessageDialog.openError(shell.getShell(), "Category Error", "An error occurred while editing the category.");
					}
	...
	public boolean enable(@Optional @Named(IServiceConstants.ACTIVE_SELECTION) Object object,
		...
		if (object instanceof CategoryModel) {
			...
			if (category.isIsPipeline() && activePart.getLabel().toLowerCase().equals(Constants.PIPELINE_LABEL)
					&& Activator.isAdmin()) {
				return true;
